...
self.addEventListener('fetch',e=>{const u=new URL(e.request.url);if(u.origin!==self.location.origin)return;const scopePath=new URL(self.registration.scope).pathname.replace(/\/+$/,'/');if(!u.pathname.startsWith(scopePath))return;const rel=u.pathname.slice(scopePath.length).split('/').filter(Boolean);const kind=rel[0]||'';const id=decodeURIComponent(rel[1]||'');const qs=u.searchParams;let body, len, withLen=true, name='probe.bin';
